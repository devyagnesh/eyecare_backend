...
class UpdateEyeExaminationRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $odCylinder = $this->has('od_cylinder') ? $this->od_cylinder : null;
            $osCylinder = $this->has('os_cylinder') ? $this->os_cylinder : null;
            $odAxis = $this->has('od_axis') ? $this->od_axis : null;
            $osAxis = $this->has('os_axis') ? $this->os_axis : null;

            if (!empty($odCylinder) && (float)$odCylinder != 0.00 && empty($odAxis)) {
                $validator->errors()->add(
                    'od_axis',
            ...
            if (!empty($osCylinder) && (float)$osCylinder != 0.00 && empty($osAxis)) {
                $validator->errors()->add(
                    'os_axis',
            ...
            if ($this->has('pd_distance') && $this->has('pd_near')) {
                $pdDistance = $this->pd_distance;
                $pdNear = $this->pd_near;
                
                if (!empty($pdDistance) && !empty($pdNear) && (float)$pdNear >= (float)$pdDistance) {
                    $validator->errors()->add(
                        'pd_near',
            ...
            if ($this->has('add_power') && !empty($this->add_power) && (float)$this->add_power < 0.00) {
                $validator->errors()->add(
                    'add_power',
